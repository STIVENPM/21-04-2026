...
CREATE TABLE aeropuerto.airport (
    airport_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    address_id uuid NOT NULL REFERENCES geografia.address(address_id),
...
CREATE TABLE aeropuerto.terminal (
    terminal_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    airport_id uuid NOT NULL REFERENCES aeropuerto.airport(airport_id),
...
CREATE TABLE aeropuerto.boarding_gate (
    boarding_gate_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    terminal_id uuid NOT NULL REFERENCES aeropuerto.terminal(terminal_id),
...
CREATE TABLE aeropuerto.runway (
...
CREATE TABLE aeropuerto.airport_regulation (
