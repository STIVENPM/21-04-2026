...
CREATE TABLE abordaje.boarding_group (
...
CREATE TABLE abordaje.check_in_status (
...
CREATE TABLE abordaje.check_in (
    ...
    check_in_status_id uuid NOT NULL REFERENCES abordaje.check_in_status(check_in_status_id),
    boarding_group_id uuid REFERENCES abordaje.boarding_group(boarding_group_id),
    checked_in_by_user_id uuid REFERENCES seguridad.user_account(user_account_id),
...
CREATE TABLE abordaje.boarding_pass (
    boarding_pass_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    check_in_id uuid NOT NULL REFERENCES abordaje.check_in(check_in_id),
...
CREATE TABLE abordaje.boarding_validation (
    boarding_validation_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    boarding_pass_id uuid NOT NULL REFERENCES abordaje.boarding_pass(boarding_pass_id),
    boarding_gate_id uuid REFERENCES aeropuerto.boarding_gate(boarding_gate_id),
    validated_by_user_id uuid REFERENCES seguridad.user_account(user_account_id),
