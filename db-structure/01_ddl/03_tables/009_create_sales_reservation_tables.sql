...
CREATE TABLE ventas.reservation_status (
...
CREATE TABLE ventas.sale_channel (
...
CREATE TABLE ventas.fare_class (
    fare_class_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    cabin_class_id uuid NOT NULL REFERENCES aeronaves.cabin_class(cabin_class_id),
...
CREATE TABLE ventas.fare (
    fare_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    airline_id uuid NOT NULL REFERENCES aerolinea.airline(airline_id),
    origin_airport_id uuid NOT NULL REFERENCES aeropuerto.airport(airport_id),
    destination_airport_id uuid NOT NULL REFERENCES aeropuerto.airport(airport_id),
    fare_class_id uuid NOT NULL REFERENCES ventas.fare_class(fare_class_id),
    currency_id uuid NOT NULL REFERENCES geografia.currency(currency_id),
...
CREATE TABLE ventas.ticket_status (
...
CREATE TABLE ventas.reservation (
    reservation_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    booked_by_customer_id uuid REFERENCES clientes.customer(customer_id),
    reservation_status_id uuid NOT NULL REFERENCES ventas.reservation_status(reservation_status_id),
    sale_channel_id uuid NOT NULL REFERENCES ventas.sale_channel(sale_channel_id),
...
CREATE TABLE ventas.reservation_passenger (
    reservation_passenger_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    reservation_id uuid NOT NULL REFERENCES ventas.reservation(reservation_id),
...
CREATE TABLE ventas.ticket (
    ...
    sale_id uuid NOT NULL REFERENCES ventas.sale(sale_id),
    reservation_passenger_id uuid NOT NULL REFERENCES ventas.reservation_passenger(reservation_passenger_id),
    fare_id uuid NOT NULL REFERENCES ventas.fare(fare_id),
    ticket_status_id uuid NOT NULL REFERENCES ventas.ticket_status(ticket_status_id),
...
CREATE TABLE ventas.ticket_segment (
    ticket_segment_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ticket_id uuid NOT NULL REFERENCES ventas.ticket(ticket_id),
    flight_segment_id uuid NOT NULL REFERENCES operaciones.flight_segment(flight_segment_id),
...
CREATE TABLE ventas.seat_assignment (
    ...
    aircraft_seat_id uuid NOT NULL REFERENCES aeronaves.aircraft_seat(aircraft_seat_id),
    ...
    CONSTRAINT fk_seat_assignment_ticket_segment FOREIGN KEY (ticket_segment_id, flight_segment_id)
        REFERENCES ventas.ticket_segment(ticket_segment_id, flight_segment_id)
);

CREATE TABLE ventas.baggage (
    baggage_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ticket_segment_id uuid NOT NULL REFERENCES ventas.ticket_segment(ticket_segment_id),
