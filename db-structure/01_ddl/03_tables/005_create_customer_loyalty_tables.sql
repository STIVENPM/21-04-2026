...
CREATE TABLE clientes.customer_category (
...
CREATE TABLE clientes.benefit_type (
...
CREATE TABLE clientes.loyalty_program (
    ...
    default_currency_id uuid NOT NULL REFERENCES geografia.currency(currency_id),
...
CREATE TABLE clientes.loyalty_tier (
    loyalty_tier_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    loyalty_program_id uuid NOT NULL REFERENCES clientes.loyalty_program(loyalty_program_id),
...
CREATE TABLE clientes.customer (
    ...
    customer_category_id uuid REFERENCES clientes.customer_category(customer_category_id),
...
CREATE TABLE clientes.loyalty_account (
    loyalty_account_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    customer_id uuid NOT NULL REFERENCES clientes.customer(customer_id),
    loyalty_program_id uuid NOT NULL REFERENCES clientes.loyalty_program(loyalty_program_id),
...
CREATE TABLE clientes.loyalty_account_tier (
    loyalty_account_tier_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    loyalty_account_id uuid NOT NULL REFERENCES clientes.loyalty_account(loyalty_account_id),
    loyalty_tier_id uuid NOT NULL REFERENCES clientes.loyalty_tier(loyalty_tier_id),
...
CREATE TABLE clientes.miles_transaction (
...
CREATE TABLE clientes.customer_benefit (
    customer_benefit_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    customer_id uuid NOT NULL REFERENCES clientes.customer(customer_id),
    benefit_type_id uuid NOT NULL REFERENCES clientes.benefit_type(benefit_type_id),
