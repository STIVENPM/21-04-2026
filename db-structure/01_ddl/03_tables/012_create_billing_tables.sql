...
CREATE TABLE facturacion.exchange_rate (
    exchange_rate_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    from_currency_id uuid NOT NULL REFERENCES geografia.currency(currency_id),
    to_currency_id uuid NOT NULL REFERENCES geografia.currency(currency_id),
...
CREATE TABLE facturacion.invoice_status (
...
CREATE TABLE facturacion.invoice (
    invoice_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    sale_id uuid NOT NULL REFERENCES ventas.sale(sale_id),
    invoice_status_id uuid NOT NULL REFERENCES facturacion.invoice_status(invoice_status_id),
...
CREATE TABLE facturacion.invoice_line (
    invoice_line_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    invoice_id uuid NOT NULL REFERENCES facturacion.invoice(invoice_id),
    tax_id uuid REFERENCES facturacion.tax(tax_id),
